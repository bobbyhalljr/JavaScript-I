...
const newArray = inventory.reduce((last) =>
...
console.log(carModels = inventory.sort(function(a, b){
...
carYears = inventory.map((carYears) => {
...
oldCars = inventory.filter((old) => {
    return old.car_year < "2000";
    return 'none';
});
...
BMWAndAudi = inventory.filter((dreamCar) => {
    if(dreamCar.car_make === "BMW"){return dreamCar}
    if(dreamCar.car_make === "Audi"){return dreamCar}
});
